                            ...
                            <div class="item">
                                <div class="item__avatar">
                                    <a href="{{ route ('get.product.detail', $product->pro_slug. '-'.$product->id)}}" title="" class="image cover">
                                        <img class="lazyload lazy" src="{{  asset('images/preloader.gif') }}" alt="Whey protein" data-src="{{ pare_url_file($product->pro_avatar) }}">
                                    </a>
                                </div>
                                <div class="item__info">
                                    <a href="#" title="Sữa tăng cân" class="cate">{{ $product->category->c_name ?? "[N\A]"}}</a>
                                    @if ($product->pro_sale)
                                        <a href="" title="SaleOff" class="cate sale">-{{ $product->pro_sale}}%</a>
                                    @endif
                                    <a href="{{ route ('get.product.detail', $product->pro_slug. '-'.$product->id)}}" title="Sữa tăng cân" class="name">{{ $product->pro_name}}</a>
                                    @if ($product->pro_sale)
                                            <p class="price">
                                            <span>Giá bán: </span>
